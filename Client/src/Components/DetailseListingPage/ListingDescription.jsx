...
export default function ListingDescription({ description }) {
  ...
  return (
    <div className="border-t border-[#e7e2d7] pt-8">
      <h2 className="mb-4 text-xl font-black text-[#183d37]">عن العقار</h2>
      <motion.p
        ref={textRef}
        animate={{ height: expanded || !needsToggle ? "auto" : 132 }}
        transition={{ duration: 0.35, ease: "easeInOut" }}
        className="overflow-hidden whitespace-pre-line leading-8 text-[#6b7d76]"
      >
        {description}
      </motion.p>
      ...
      {needsToggle && (
        <button
          onClick={() => setExpanded((v) => !v)}
          className="mt-3 flex items-center gap-1 text-sm font-bold text-[#e49263] hover:text-[#c9763f]"
        >
          {expanded ? "عرض أقل" : "قراءة المزيد"}
          <motion.span
            animate={{ rotate: expanded ? 180 : 0 }}
