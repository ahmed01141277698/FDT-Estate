...
// حقول بيتحكم فيها السيرفر فقط ولازم متدخلش الفورم أبدًا — سواء وقت التعديل
// أو وقت الحفظ في localStorage، عشان منبعتهاش بالغلط مع إنشاء عقار جديد.
...
function stripServerFields(obj) {
  const clean = { ...obj };
  SERVER_MANAGED_FIELDS.forEach((field) => delete clean[field]);
  return clean;
}
...
export default function useListingForm({ editId = null, initialData = null } = {}) {
  ...
  const [form, setForm] = useState(() => {
    const savedForm = localStorage.getItem("listingForm");
    if (initialData) return { ...initialForm, ...stripServerFields(initialData) };
    return savedForm ? stripServerFields(JSON.parse(savedForm)) : initialForm;
  });
  ...
  useEffect(() => {
    // نتأكد إن أي حقول سيرفر ما تتخزنش في localStorage حتى لو دخلت الفورم
    // بأي طريقة تانية غير المتوقعة.
    localStorage.setItem("listingForm", JSON.stringify(stripServerFields(form)));
  }, [form]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // stripServerFields هنا هي الضمانة النهائية: حتى لو _id أو أي حقل سيرفر
      // اتسرب لأي سبب لسه في form، مش هيوصل للـ payload المُرسل للباك إند.
      const payload = {
        ...stripServerFields(form),
        price: Number(form.price),
        discountPrice: form.offer ? Number(form.discountPrice || 0) : 0,
        bedrooms: Number(form.bedrooms),
        bathrooms: Number(form.bathrooms),
