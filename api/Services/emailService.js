...
const FROM = `"${process.env.EMAIL_FROM_NAME || "مَسكَن"}" <${process.env.EMAIL_FROM || process.env.EMAIL_USER}>`;
...
function buildPasswordResetText({ username, otp, expirationMinutes }) {
  return `طلب استعادة كلمة المرور - مَسكَن
...
export async function sendPasswordResetEmail({
  ...
  await transporter.sendMail({
    from: FROM,
    to,
    subject: "طلب استعادة كلمة المرور - مسكن",
...
function buildPasswordChangedText({ username, changedAt }) {
  return `تم تغيير كلمة المرور - مَسكَن
...
export async function sendPasswordChangedEmail({ to, username, changedAt }) {
  await transporter.sendMail({
    from: FROM,
    to,
    subject: "تم تغيير كلمة المرور الخاصة بحسابك - مسكن",
